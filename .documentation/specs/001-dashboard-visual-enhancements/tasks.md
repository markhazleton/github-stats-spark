...
- [ ] T001 Bump schema version from `"2.2.0"` to `"2.3.0"` in `src/spark/unified_data_generator.py`
...
- [ ] T004 [P] Add `fetch_contributor_stats()` method to `src/spark/fetcher.py` — calls `repo.get_stats_contributors()`, retries 202/rate-limit responses with explicit 1s/2s/4s/8s backoff, emits timestamped degradation logs, and returns `None` only after logged fallback; returns contributor dicts with login/commits/additions/deletions when available
- [ ] T005 [P] Add `fetch_code_frequency()` method to `src/spark/fetcher.py` — calls `repo.get_stats_code_frequency()`, retries 202/rate-limit responses with explicit 1s/2s/4s/8s backoff, emits timestamped degradation logs, and returns `None` only after logged fallback; returns total additions and total deletions when available
...
- [ ] T009 Run `spark unified --user markhazleton --verbose` and verify new fields appear in `data/repositories.json` with correct values, degraded repositories are marked explicitly, and fallback logging is emitted when stats are unavailable
...
- [ ] T036 [P] [US1] Add Vitest coverage for the heatmap in `frontend/tests/ContributionHeatmap.test.jsx` — verify trailing-365-day cell generation, tooltip content, and empty-calendar rendering
...
- [ ] T037 [P] [US2] Add Vitest coverage for the timeline in `frontend/tests/ActivityTimeline.test.jsx` — verify both datasets render, legend toggles work, and tooltip values match the weekly input data
...
- [ ] T038 [P] [US3] Add Vitest coverage for the theme toggle in `frontend/tests/ThemeToggle.test.jsx` — verify persisted preference, system fallback behavior, and `data-theme` updates on `<html>`
...
- [ ] T025 [US4] Add new export column definitions for `total_additions`, `total_deletions`, `code_churn`, `bus_factor`, and `bus_factor_health` to `frontend/src/components/Common/ExportButton.jsx` — place new columns logically alongside existing commit metrics, preserve filter-scoped export behavior, and keep CSV escaping RFC 4180-compliant
- [ ] T039 [US4] Verify JSON and CSV export behavior from `frontend/src/components/Common/ExportButton.jsx` — confirm filtered rows only are exported, special characters remain properly escaped, and the CSV opens cleanly in spreadsheet tools
...
- [ ] T026 [US5] Add commit volume display (total_additions, total_deletions, code_churn) to the profile summary stats section in `frontend/src/components/Visualizations/StatCards.jsx`
...
- [ ] T040 [P] Verify dashboard heatmap performance — measure the contribution heatmap on representative data and confirm it becomes usable within 2 seconds of page load (SC-001)
- [ ] T041 [P] Verify theme-switch interaction quality — confirm the dashboard theme changes in under 200ms with no observable layout shift (SC-003)
- [ ] T042 [P] Verify data-quality targets — confirm commit volume metrics populate for at least 80% of repositories under authenticated rate limits and manually spot-check bus factor results for representative repositories to stay within the 1% tolerance target (SC-005, SC-006)
