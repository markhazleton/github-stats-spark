...
1. Run targeted unit tests for remediation-sensitive areas:
...
1. Run coverage for the core Python package and confirm the visualization module meets the constitutional threshold:
...
1. Run a bounded unified workflow to verify configuration-driven output and aggregate totals without full-scale execution:
...
1. Confirm the active feature artifacts are stored under `.documentation/specs/001-remediate-high-issues/` and not in a legacy root `specs/` directory.
...
1. Re-run the site audit after implementation and confirm the in-scope HIGH findings are cleared.
