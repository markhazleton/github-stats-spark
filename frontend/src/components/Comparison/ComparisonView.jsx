...
import { formatNumber, formatDate, calculatePercentageDifference } from '@/services/metricsCalculator'
...
function ComparisonView({ repositories, onRemoveRepo }) {
  /**
   * Metrics to compare across repositories
   */
  const comparisonMetrics = [
    ...
    { key: 'commit_history.total_commits', label: 'Total Commits', type: 'number' },
    { key: 'commit_history.recent_90d', label: 'Commits (90d)', type: 'number' },
    { key: 'commit_metrics.avg_size', label: 'Avg Commit Size', type: 'number' },
    { key: 'commit_velocity', label: 'Commit Velocity', type: 'number', decimals: 2 },
  ...
  const comparisonData = useMemo(() => {
    ...
    return comparisonMetrics.map(metric => {
      const values = repositories.map(repo => getNestedValue(repo, metric.key))
  ...
  return (
    <div className={styles.comparisonView}>
        ...
        {comparisonData.map((item, rowIndex) => (
          <div key={item.metric.key} className={styles.metricRow}>
            ...
            {item.values.map((value, colIndex) => {
              ...
              const percentDiff = item.metric.type === 'number' && item.max != null && value != null
      ...
      <div className={styles.legend}>
        ...
        <div className={styles.legendItem}>
          <span className={styles.legendNote}>Percentage shows difference from maximum value</span>
        </div>
      </div>
    </div>
  )
