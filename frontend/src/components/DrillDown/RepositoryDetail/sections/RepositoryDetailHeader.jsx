...
function RepositoryDetailHeader({ repository, onClose, formatNumber }) {
  ...
  const activityLabel =
    ...
    : daysSincePush === 0 ? "Today"
    : daysSincePush === 1 ? "Yesterday"
    : daysSincePush <= 7 ? `${daysSincePush}d ago`
    : daysSincePush <= 30 ? `${Math.round(daysSincePush / 7)}w ago`
    : daysSincePush <= 365 ? `${Math.round(daysSincePush / 30)}mo ago`
        ...
        <div className={styles.headerContent}>
          ...
          {repository.topics?.length > 0 && (
            <div className={styles.headerTopics}>
              {repository.topics.slice(0, 8).map((topic) => (
                <span key={topic} className={styles.headerTopic}>{topic}</span>
              ))}
          ...
          <div className={styles.headerStats}>
            <span className={styles.headerStat} title="Stars">
              <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className={styles.headerStatIcon}>
                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
              </svg>
              {formatNumber(repository.stars)}
            </span>
            <span className={styles.headerStat} title="Forks">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true" className={styles.headerStatIcon}>
