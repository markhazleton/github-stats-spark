...
const MATURITY_CONFIG = {
  ...
  "active-development": { label: "Active Dev", color: "#3b82f6", bg: "rgba(59, 130, 246, 0.1)" },
  "experimental": { label: "Experimental", color: "#f59e0b", bg: "rgba(245, 158, 11, 0.1)" },
...
function QualityBadge({ icon, label, active }) {
  return (
    <span className={`${styles.qualityBadge} ${active ? styles.qualityBadgeActive : styles.qualityBadgeInactive}`} title={label}>
      {icon}
    </span>
  );
}
...
function RepoCard({ repo, onClick }) {
  ...
  const summaryExcerpt = summary.length > 140 ? summary.slice(0, 137) + "…" : summary;

  const daysSincePush = repo.days_since_last_push;
  const activityLabel = daysSincePush === 0
    ? "Today"
    : daysSincePush === 1
    ? "Yesterday"
    : daysSincePush <= 7
  ...
  return (
    <article
      ...
      onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onClick?.(repo); } }}
    >
          ...
          <div className={styles.cardMeta}>
            ...
            {maturityConfig && (
              <span
                ...
                style={{ color: maturityConfig.color, background: maturityConfig.bg, border: `1px solid ${maturityConfig.color}30` }}
              >
                {maturityConfig.label}
              </span>
            )}
            {isRecent && (
              <span className={styles.recentBadge}>🔥 Hot</span>
            )}
          </div>
        </div>
        ...
        <div className={styles.cardFooter}>
          <div className={styles.footerStats}>
            <span className={styles.stat} title="Stars">
              <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className={styles.statIcon}>
                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
              </svg>
              {repo.stars}
            </span>
            <span className={styles.stat} title="Forks">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true" className={styles.statIcon}>
            ...
            <span className={`${styles.stat} ${isRecent ? styles.statRecent : ""}`} title={`Last push ${activityLabel}`}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true" className={styles.statIcon}>
          ...
          <div className={styles.qualityIndicators}>
            <QualityBadge icon="📄" label="Has README" active={repo.has_readme} />
            <QualityBadge icon="⚖️" label="Has License" active={repo.has_license} />
...
export default function RepositoryGrid({ repositories, onRepoClick }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterMaturity) {
      list = list.filter((r) => r.ai_summary?.project_maturity === filterMaturity);
    }

    list.sort((a, b) => {
      switch (sortBy) {
        case "stars": return (b.stars || 0) - (a.stars || 0);
        case "activity": return (a.days_since_last_push ?? 999) - (b.days_since_last_push ?? 999);
        case "commits": return (b.total_commits || 0) - (a.total_commits || 0);
        case "name": return a.name.localeCompare(b.name);
        case "age": return new Date(b.created_at) - new Date(a.created_at);
        default: return 0;
      }
    });
  ...
  const maturities = useMemo(() => {
    const m = new Set(repositories.map((r) => r.ai_summary?.project_maturity).filter(Boolean));
    return Array.from(m);
  }, [repositories]);
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.toolbarControls}>
          ...
          >
            ...
            {maturities.map((m) => (
              <option key={m} value={m}>{MATURITY_CONFIG[m]?.label || m}</option>
            ))}
          </select>
      ...
      {(filterLang || filterMaturity || search) && (
        <div className={styles.activeFilters}>
          ...
          <button
            ...
            onClick={() => { setSearch(""); setFilterLang(""); setFilterMaturity(""); }}
          >
            Clear all filters
          </button>
        </div>
