 ...
 * @returns {React.LazyComponent} - Lazy loaded component
 *
 * @example
...
export const lazyLoadComponent = (importFn) => {
  // Note: This function is deprecated. Use React.lazy() directly instead.
...
export const preloadResource = (url, type, crossOrigin = null) => {
  ...
  // Add crossorigin for fonts and fetch (required for CORS)
  if (crossOrigin) {
    link.crossOrigin = crossOrigin;
  } else if (type === "fetch" || type === "font") {
    // Default to 'anonymous' for fetch and font resources to match fetch credentials
